...
Future<void> handleGDPRConsent() async {
  ...
  ConsentInformation.instance.requestConsentInfoUpdate(params, () async {
    print("GDPR Consent: Consent info updated. Status: ${await ConsentInformation.instance.getConsentStatus()}");
    if (await ConsentInformation.instance.isConsentFormAvailable()) {
      print("GDPR Consent: Consent form is available. Loading form...");
      await _loadAndShowConsentForm(); // Added await here
    } else {
      print("GDPR Consent: Consent form not available or not required at this time.");
      await _completeConsentFlowAndInitializeAds(); // Added await here
    }
  }, (FormError error) async { // Added async here
    print('GDPR Consent: Error requesting consent info update: ${error.message}');
...
Future<void> _loadAndShowConsentForm() async { // Changed to Future<void> and made async
   ConsentForm.loadConsentForm( // Added await
        (ConsentForm consentForm) async {
      print("GDPR Consent: Form loaded. Current status: ${await ConsentInformation.instance.getConsentStatus()}");
      ...
      if (await ConsentInformation.instance.getConsentStatus() == ConsentStatus.required) {
        ...
        consentForm.show((FormError? formError) async { // Made callback async
          if (formError != null) {
            print('GDPR Consent: Error showing consent form: ${formError.message}');
          }
        ...
        (FormError? formError) async { // Made callback async
      print('GDPR Consent: Error loading consent form: ${formError?.message}');
...
Future<void> _completeConsentFlowAndInitializeAds() async { // Changed to Future<void>
  ...
  print("GDPR Consent: Showing privacy options form...");
   ConsentForm.loadConsentForm( // Added await
       ...
       consentForm.show(
            (FormError? formError) {
          if (formError != null) {
            print('GDPR Consent: Error showing privacy options form: ${formError.message}');
          }
        ...
        (FormError? formError) {
      print('GDPR Consent: Error loading privacy options form: ${formError?.message}');
    },
  );
}
