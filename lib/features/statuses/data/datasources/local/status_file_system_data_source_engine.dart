...
import 'package:storysaver/features/permissions/domain/repositories/permission_repository.dart';
import 'thumbnail_generator.dart';
// import 'package:saf/saf.dart';

class StatusFileSystemDataSourceEngine {
  StatusFileSystemDataSourceEngine({required PermissionBaseRepo permissionRepo})
    : _permissionRepo = permissionRepo;

  final PermissionBaseRepo _permissionRepo;
    ...
    try {
      ...
      for (final permission in accessiblePath) {
        ...
        if (decodedUri.contains("whatsapp.w4b") &&
          ...
          statusDir = await DocumentFile.fromUri(permission.uri).catchError((
            error,
          ) async {
            print('Error listing documents: $error ${permission.uri}');
      ...
      if (statusDir == null) {
        ...
        for (final permission in accessiblePath) {
          ...
          if (decodedUri.contains("Android") &&
            ...
            if (androidMediaDir != null && await androidMediaDir.exists) {
              statusDir = await _navigateToWhatsAppBusinessStatus(
                androidMediaDir,
              );
              if (statusDir != null) {
                print(
  ...
  Future<DocumentFile?> _navigateToWhatsAppBusinessStatus(
    ...
    try {
      ...
      DocumentFile? whatsappW4bDir = mediaContents
          .cast<DocumentFile?>()
          .firstWhere(
      ...
      DocumentFile? whatsappBusinessDir = w4bContents
          .cast<DocumentFile?>()
          .firstWhere(
      ...
      DocumentFile? mediaDir = businessContents
          .cast<DocumentFile?>()
          .firstWhere(
  ...
  Future<bool> _requestStoragePermission() async {
    final result = await _permissionRepo.requestStoragePermission();
