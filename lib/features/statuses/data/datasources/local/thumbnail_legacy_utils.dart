  ...
  for (final file in files) {
    try {
      ...
      if (thumbnail != null) {
        ...
        yield* Stream<Uint8List>.error(
          'Failed to generate thumbnail for ${file.path}',
        );
      }
    } catch (error) {
      yield* Stream<Uint8List>.error(
        'Error generating thumbnail for ${file.path}: $error',
      );
    }
  }
}
...
Future<Uint8List?> generateThumbnailInIsolateWithFlutterIsolates(
  String videoPath,
) async {
  final receivePort = ReceivePort();

  // Spawn the isolate
  await FlutterIsolate.spawn(_generateThumbnailTask, [
    videoPath,
